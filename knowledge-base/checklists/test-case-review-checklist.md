...
| Every acceptance criterion is mapped to at least one test case in a coverage matrix. | ☐ |
| Every business rule is mapped to at least one test case or marked Not Applicable with evidence. | ☐ |
| Every applicable NFR is mapped to at least one test case. | ☐ |
| Every clarified behavior decision is reflected in a test step, test data item, or expected result. | ☐ |
| No test case validates behavior removed or superseded by the current requirement. | ☐ |
...
# 11. Integration Coverage Review

Identify and document cross-feature integration requirements:

| Check | Status |
|-------|--------|
| All within-feature dependencies are covered in functional test cases. | ☐ |
| Cross-feature integration touchpoints are identified. | ☐ |
| If cross-feature integration exists, separate integration test cases are recommended or documented. | ☐ |
| Integration test cases follow naming convention: `TC_INT_<SEQUENCE>`. | ☐ |
| Integration test cases are placed in `test-cases/integration/` or clearly marked as integration scope. | ☐ |

### Integration Examples to Consider:

- **Create → Read:** Add User → View User List, Create Order → View Order History
- **Create → Search:** Add User → Search User, Add Product → Search Product  
- **Create → Update:** Add User → Edit User
- **Create → Delete:** Add User → Delete User
- **Multi-step workflows:** Login → Select Product → Checkout → Payment
- **Cross-module flows:** User Management → Reporting, Inventory → Sales

### Decision Guidance:

- If behavior validates **one feature only**, include in functional test cases for that feature.
- If behavior validates **two or more features working together**, create separate integration test case.
- Document integration test recommendations in test case review notes or coverage report.
...
| Requirement references use the local AC/BR/NFR/Clarified Behavior format when the file represents one User Story. | ☐ |
| A coverage matrix identifies Covered, Partial, and Missing requirements. | ☐ |
...
| Requirement-analysis readiness is considered before declaring the test set review complete. | ☐ |
...
- A requirement coverage matrix is included in the review evidence.
- Clarified requirements are checked against the current requirement version.
- Removed, obsolete, or superseded behavior is not covered by remaining test cases.